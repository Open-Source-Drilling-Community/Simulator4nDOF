...
namespace NORCE.Drilling.Simulator4nDOF.Simulator.SimulatorModels
{
    ...
    {             
        ...
        public WaveModel(State state, in SimulationParameters simulationParameters)
        {
            //Needs to be update after testing stage
            ElementLength = 4 * simulationParameters.Drillstring.TotalLength / simulationParameters.LumpedCells.NumberOfLumpedElements;
            // Calculate the number of elements based on the total length and element length, ensuring it's an integer
            NumberOfElements = (int) Math.Ceiling(simulationParameters.Drillstring.TotalLength / ElementLength);
            // Recalculate the element length to fit the total length exactly
            ElementLength = simulationParameters.Drillstring.TotalLength / NumberOfElements;
            ...
            InterpolatedStrain = Vector<double>.Build.Dense(state.ZVelocity.Count);
            InterpolatedVelocity = Vector<double>.Build.Dense(state.ZVelocity.Count);
